...
public class FakeDataLoremService(IFakerService fakerService) : IFakeDataLoremService
{
    ...
    {
        LoremProperty.WORD => _fakerService.Lorem.Word(),
        ...
        LoremProperty.TEXT => _fakerService.Lorem.Text(),
    ...
    private string GenerateSentence(Dictionary<string, object> parameters)
    {
        ...
        return _fakerService.Lorem.Sentence(wordCount, range);
    }
    ...
    private string GenerateParagraphs(Dictionary<string, object> parameters)
    {
        ...
        if (parameters.ContainsKey(PARAM_MIN) && parameters.ContainsKey(PARAM_MAX))
        {
            ...
            return _fakerService.Lorem.Paragraphs(min, max, separator);
        }
        else
        {
            // Override 1: count, separator
            var count = parameters.ConvertToInt(PARAM_COUNT, 3);
            return _fakerService.Lorem.Paragraphs(count, separator);
        }
    }
